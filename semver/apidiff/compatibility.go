...
func (d *differ) checkCompatibleChan(otn *types.TypeName, old, new *types.Chan) {
	d.checkCorrespondence(otn, ", element", old.Elem(), new.Elem())
	if old.Dir() != new.Dir() {
		if new.Dir() == types.SendRecv {
			d.compatible(otn, "", typeKindString(old), "removed direction")
		} else {
			d.incompatible(otn, "", typeKindString(old), "changed direction")
		}
	}
}
...
func (d *differ) checkCompatibleBasic(otn *types.TypeName, old, new *types.Basic) {
	...
	if compatibleBasics[[2]types.BasicKind{old.Kind(), new.Kind()}] {
		d.compatible(otn, "", typeKindString(new), "changed from %s to %s", old, new)
	} else {
		d.typeChanged(otn, "", "", old, new)
	}
}
...
func (d *differ) checkCompatibleInterface(otn *types.TypeName, old, new *types.Interface) {
	...
	if unexportedMethod(old) != nil {
		...
		if u := unexportedMethod(new); u != nil {
			d.incompatible(otn, u.Name(), typeKindString(old), "added unexported method")
		}
	}
}
...
func (d *differ) checkCompatibleStruct(obj types.Object, old, new *types.Struct) {
	...
	if types.Comparable(old) && !types.Comparable(new) {
		d.incompatible(obj, "", typeKindString(old), "old is comparable, new is not")
	}
}
...
func (d *differ) checkCompatibleObjectSets(obj types.Object, old, new map[string]types.Object) {
	for name, oldo := range old {
		newo := new[name]
		if newo == nil {
			d.incompatible(obj, name, typeKindString(oldo.Type()), "removed")
		} else {
			d.checkCorrespondence(obj, name, oldo.Type(), newo.Type())
		}
	}
	for name, newo := range new {
		if old[name] == nil {
			d.compatible(obj, name, typeKindString(newo.Type()), "added")
		}
	}
}
...
func (d *differ) checkMethodSet(otn *types.TypeName, oldt, newt types.Type, addcompat bool) {
	...
	for name, oldMethod := range oldMethodSet {
		...
		if newMethod == nil {
			...
			if part == "" {
				part = fmt.Sprintf(", %s method", strings.TrimPrefix(typeKindString(receiverNamedType(oldMethod)), "named|"))
			}
			d.incompatible(oldMethod, part, typeKindString(oldMethod.Type()), "removed")
		} else {
			obj := oldMethod
			...
			part := fmt.Sprintf(", %s method", strings.TrimPrefix(typeKindString(receiverNamedType(obj)), "named|"))
			d.checkCorrespondence(obj, part, oldMethod.Type(), newMethod.Type())
		}
	}
	...
	for name, newMethod := range newMethodSet {
		if oldMethodSet[name] == nil {
			part := fmt.Sprintf(", %s method", strings.TrimPrefix(typeKindString(receiverNamedType(newMethod)), "named|"))
			if addcompat {
				d.compatible(newMethod, part, typeKindString(newMethod.Type()), "added")
			} else {
				d.incompatible(newMethod, part, typeKindString(newMethod.Type()), "added")
			}
		}
	}
}
